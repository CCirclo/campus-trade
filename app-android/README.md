...
| 存储 | SharedPreferences(会话 Cookie)、内存 StateFlow(用户态) |
...
- **商品**:首页列表(搜索 / 分类筛选 / 三种排序)、详情(图库轮播、卖家卡、收藏、评论、举报、『我想要』发起会话)、用户公开主页
...
```bash
cd ../frontend && npm install && npm run dev   # API 监听 :8787
```

## 会话机制说明

后端使用 **httpOnly Cookie 会话**(`campus_session`,30 天)。原生端通过 OkHttp 的
`CookieJar` 捕获 `Set-Cookie` 并持久化到 SharedPreferences,进程重启后自动恢复;
无需在代码中处理任何 token。

## 已知限制(首版)

- 商品列表沿用后端 `LIMIT 100` 一次加载,暂未做分页/无限滚动
