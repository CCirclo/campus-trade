...
export default function AdminReward() {
  ...
  const setBonus = (code: 'lungmen' | 'originium', value: number) => setSettings(s => ({ ...s, signupBonus: { ...s.signupBonus, [code]: value } }));
      ...
      <form className="admin-section" onSubmit={save}>
        ...
        <div className="admin-form-grid">
          ...
          <div className="admin-form-grid two">
            <label>原石数量<input type="number" min={0} value={settings.signupBonus.lungmen} onChange={e => setBonus('lungmen', Number(e.target.value))} /></label>
            <label>创世结晶数量<input type="number" min={0} value={settings.signupBonus.originium} onChange={e => setBonus('originium', Number(e.target.value))} /></label>
          </div>
          <div className="admin-section-head"><Coins /><div><b>发布与购买奖励</b><small>每发布或购买一件商品奖励的原石数量</small></div></div>
          <div className="admin-form-grid two">
            <label>发布奖励（原石）<input type="number" min={0} value={settings.publishReward} onChange={e => setSettings(s => ({ ...s, publishReward: Number(e.target.value) }))} /></label>
            <label>购买奖励（原石）<input type="number" min={0} value={settings.purchaseReward} onChange={e => setSettings(s => ({ ...s, purchaseReward: Number(e.target.value) }))} /></label>
          </div>
