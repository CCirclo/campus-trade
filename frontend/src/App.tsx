...
function ItemCard({item,attribution}:{item:Item;attribution?:Attribution}){
  ...
  return <Link ref={ref} className={`item-card${item.kind==='贴图'?' pic':''}`} to={`/items/${item.id}`} onClick={()=>{if(attribution){saveAttribution(item.id,attribution);track('item_click',attribution,{itemId:item.id})}}}>
    ...
    <div className="item-card-body"><h3>{item.title}</h3><p className="item-meta-line"><span>{item.category} · {(item.regions||[]).join(' / ')}</span><time dateTime={item.createdAt}>发布于 {formatTimestamp(item.createdAt)}</time></p><div><strong className={item.kind==='贴图'?'pic-price':''}>{item.kind==='贴图'?item.price:formatItemPrices(item)}</strong></div></div>
  </Link>;
}
