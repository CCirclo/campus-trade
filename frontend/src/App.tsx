...
function LoginPage(){
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError('');try{await post(`/api/auth/${mode}`,{email,password,nickname,code,campusId,emailMessageNotifications});await refresh();navigate(params.get('next')||'/');}catch(e){setError(e instanceof Error?e.message:'登录失败')}finally{setBusy(false)}};
