...
function Shell({children}:{children:ReactNode}){
  ...
  return <div className="app-shell">
    <header className="site-header"><div className="header-inner">
      ...
      <div className="header-actions">{user&&currentSchool?.campuses.length?<label className={`campus-switcher${switchingCampus?' busy':''}`} title="切换当前浏览和发布校区"><MapPin className="campus-pin"/><span>{user.schoolName} ·</span><strong>{user.campusName}</strong><ChevronsUpDown className="switch-cue" aria-hidden="true"/><select value={user.campusId} onChange={e=>void switchCampus(e.target.value)} disabled={switchingCampus} aria-label="切换校区">{currentSchool.campuses.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>:<span className="school-chip">{scopeName}</span>}{user?<Link className="user-chip" to="/mine"><img src={avatar(user.avatarUrl)} alt=""/><span>{user.nickname}</span></Link>:<Link className="button primary compact" to="/login">登录</Link>}<button className="icon-button menu-button" onClick={()=>setMenu(v=>!v)} aria-label="打开菜单"><Menu/></button></div>
...
function LoginPage(){
  ...
  return <div className="auth-page"><div className="auth-visual"><Link to="/" className="brand light"><span className="brand-mark">集</span><span><b>校园闲置</b><small>同校好物 · 当面流转</small></span></Link><div><span className="eyebrow light">WELCOME BACK</span><h1>好物还在等你，<br/>同学也在。</h1><p>登录后发布闲置、收藏商品，与同校同学放心沟通。</p></div><div className="auth-proof"><ShieldCheck/><span><b>隐私与安全优先</b><small>邮箱验证码短时有效，密码只保存不可逆哈希。</small></span></div></div>
    <div className="auth-panel"><Link to="/" className="close-link" aria-label="返回首页"><X/></Link><div className="auth-card"><span className="eyebrow">CAMPUS ID</span><h2>{mode==='login'?'欢迎回来':'加入校园圈'}</h2><p>{selectedSchool?.name||'多学校校园闲置平台'}</p>
...
function MinePage(){
  ...
  return <div className="profile-page"><section className="profile-hero"><img src={avatar(user.avatarUrl)} alt="个人头像"/><div><span className={`verified-line ${user.campusVerified?'':'not-campus'}`}><ShieldCheck/>{user.campusVerified?'已认证校园账号':'非校园邮箱 · 仅可浏览'}</span><h1>{user.nickname}</h1><p>{user.email} · {user.schoolName} · {user.campusName}</p></div><Link className="button secondary" to="/profile"><Edit3/>编辑资料</Link></section><section className="stats-row"><div><b>{stats.total}</b><span>累计发布</span></div><div><b>{stats.selling}</b><span>正在出售</span></div><div><b>{stats.sold}</b><span>已经售出</span></div></section><section className="mine-grid"><Link to="/my-items"><span className="feature-icon blue"><Package/></span><div><b>我的发布</b><small>管理闲置与状态</small></div><ChevronRight/></Link><Link to="/favorites"><span className="feature-icon rose"><Heart/></span><div><b>我的收藏</b><small>查看感兴趣的好物</small></div><ChevronRight/></Link><Link to="/messages"><span className="feature-icon green"><MessageCircle/></span><div><b>商品消息</b><small>继续和同学沟通</small></div><ChevronRight/></Link><Link to="/feedback"><span className="feature-icon blue"><LifeBuoy/></span><div><b>问题反馈与建议</b><small>报告异常或告诉我们你的想法</small></div><ChevronRight/></Link><Link to="/safety"><span className="feature-icon amber"><ShieldCheck/></span><div><b>安全交易指南</b><small>校内面交与防骗提醒</small></div><ChevronRight/></Link><Link to="/password"><span className="feature-icon violet"><ShieldCheck/></span><div><b>账号安全</b><small>修改登录密码</small></div><ChevronRight/></Link>{user.role==='admin'&&<Link to="/admin"><span className="feature-icon violet"><ShieldCheck/></span><div><b>管理后台</b><small>管理用户、商品与举报</small></div><ChevronRight/></Link>}</section><button className="logout-button" onClick={async()=>{await logout();navigate('/')}}><LogOut/>退出登录</button></div>;
}
