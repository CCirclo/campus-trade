...
test('public seller profiles expose only privacy-safe fields',()=>{
  const profile=publicSellerProfile({id:7,nickname:'同学',avatar_url:'avatar.jpg',email:'student@ruc.edu.cn',email_verified:1,wechat_id:'private',last_seen_at:'private',password_hash:'private'});
  assert.deepEqual(profile,{id:7,nickname:'同学',avatarUrl:'avatar.jpg',campusVerified:true});
  assert.deepEqual(Object.keys(profile),['id','nickname','avatarUrl','campusVerified']);
});
