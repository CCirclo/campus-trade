...
import {canAccessSchool,canSetManualVerification,isSuperAdminEmail,SUPER_ADMIN_EMAIL} from '../server/admin-permissions.js';
...
test('campus managers may manually verify users but not other admins',()=>{assert.equal(canSetManualVerification(false,'user'),true);assert.equal(canSetManualVerification(false,'admin'),false);assert.equal(canSetManualVerification(true,'admin'),true)});
